# Zé Treino MCP server

A [Model Context Protocol](https://modelcontextprotocol.io) bridge that lets an external LLM
application (Claude Desktop, Cursor, Cline, Continue, etc.) read your Zé Treino profile —
...
session secrets — it can only read the same `state-<uid>.json` files the Zé Treino api already
writes.
...
OPENGYM_UID=<your-uid> OPENGYM_DATA=/path/to/Zé Treino/data node src/index.js
```
...
```jsonc
{
  "mcpServers": {
    "opengym": {
      "command": "node",
      "args": ["/absolute/path/to/Zé Treino/mcp/src/index.js"],
      "env": {
        "OPENGYM_DATA": "/absolute/path/to/Zé Treino/data",
...
Restart the client; you should see the Zé Treino tools appear with "serving profile \<name\>" on
the server's stderr.
...
AGPL-3.0-or-later, same as Zé Treino.
