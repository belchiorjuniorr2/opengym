...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR Zé Treino — just the
...
export function parsePlan(raw) {
  ...
  if (!data || !data.opengym_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t an Zé Treino plan file'))
  }
...
<body><div class="doc">
  <header>
    <div class="kicker">Zé Treino</div>
  ...
  ${body}
  <footer>${esc(t('Made with Zé Treino'))} · opengym.duarte-santos.ch</footer>
</div></body></html>`
}
