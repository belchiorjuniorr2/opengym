# Self-hosting Zé Treino

Zé Treino is two small containers (a web server and an API) plus a folder of your data.
...
Zé Treino signs you in with **passkeys** (WebAuthn). Browsers enforce two rules:
...
passkey prompt won't appear. To use Zé Treino from your phone you need a real HTTPS hostname.
...
Put Zé Treino behind something that terminates TLS for a hostname you control, then point it at
...
Zé Treino only needs the browser to reach it over `https://gym.example.com`.
...
The defaults assume Zé Treino is the only thing here: a service called `api` on port 3000, and nginx
...
Zé Treino can push two kinds of alert to your phone/desktop, even when the app isn't open:
...
you about your pushes. Zé Treino sends your `ORIGIN` by default; set `VAPID_SUBJECT=mailto:you@example.com`
